...
public class ArvoreBinaria<G extends Comparable<G>> {
	...
	public boolean inserir(G conteudo) {
		...
		if (this.getRaiz() == null) {
			...
			this.setRaiz(new NoTriplo<G>(conteudo));
			return true;
		}
		...
		if (conteudo.compareTo(aux2.getConteudo()) >= 0) {
			...
			aux2.setDireito(new NoTriplo<G>(conteudo));
		}

		else {

			aux2.setEsquerdo(new NoTriplo<G>(conteudo));
		}
	...
	/**
	 * Método que retorna o nó com m�ximo valor na árvore.
	 * 
	...
	private NoTriplo<G> sucessor(NoTriplo<G> no) {
		...
		/*
		 * Se o nó de origem não possuir um filho à direita, então o sucessor deverá ser
	...
	private NoTriplo<G> predecessor(NoTriplo<G> no) {
		...
		/*
		 * Se o nó de origem não possuir um filho à esquerda, então o predecessor deverá
